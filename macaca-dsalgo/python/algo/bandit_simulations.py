...
def thompson_sample(estimated_beta_params):
    successes = estimated_beta_params[:, 0]
    totals = estimated_beta_params.sum(1)
    choice = numpy.argmax(pymc.rbeta(1 + successes, 1 + totals - successes))
    return choice
...
for i in range(number_experiments):
    ...
    regret_accumulator[:, 5] += run_bandit_dynamic_alg(true_rewards, CTRs_that_generated_data, thompson_sample)
    regret_accumulator[:, 6] += run_bandit_dynamic_alg(true_rewards, CTRs_that_generated_data, thompson_sample_desc)

plt.semilogy(regret_accumulator / number_experiments)
plt.title('Simulated Bandit Performance for K = 7')
plt.ylabel('Cumulative Expected Regret')
plt.xlabel('Round Index')
plt.legend(
    ('Random', 'Naive', 'Epsilon-Greedy', '(1 - 1/t) UCB', '95% UCB', 'thompson-sample-max', 'thompson-sample-min'),
    loc='lower right')
plt.show()
